...
public partial class Pages_DepartmentTypeMaster : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Please Add Department Type');ShowPopup3();", true);
            return;
        }
        ...
        cmd.Parameters.AddWithValue("@DepartmentTypeName", txtDepartmentType.Text);
        ...
        if (result == 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Department Type Already exist');ShowPopup3();", true);
            txtDepartmentType.Text = "";
            return;
        }
        else
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Department Type Added successfully','success');$('.modal-backdrop').remove();", true);
        }
    ...
    protected void GVDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "modify")
        {
            ...
            ClearDetails();
            txtDepartmentType.Text = GVDetails.Rows[RowIndex].Cells[2].Text.Replace("&amp;","&");
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        if (txtDepartmentType.Text == "")
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Please Add Department Type');ShowPopup();", true);
            return;
        }
        ViewState["OpenFlag"] = 0;
        conn.Open();
        ...
        cmd.Parameters.AddWithValue("@DepartmentTypeCode", ViewState["DC"].ToString());
        cmd.Parameters.AddWithValue("@DepartmentTypeName", txtDepartmentType.Text);
        cmd.Parameters.AddWithValue("@EmpCode", 1);
        ...
        if (result == 1)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Department Type Updated successfully','success');$('.modal-backdrop').remove();", true);
        ...
        foreach (GridViewRow row in GVDetails.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkRow.Checked)
                {
                    ...
                    cmd = new SqlCommand("SP_DepartmentTypeMaster", conn);
                    cmd.Parameters.AddWithValue("@calltype", 4);
                    cmd.Parameters.AddWithValue("@DepartmentTypeCode", row.Cells[1].Text);
                    SqlParameter parm = new SqlParameter("@Result", SqlDbType.Int);
                    parm.Direction = ParameterDirection.ReturnValue;
                    cmd.Parameters.Add(parm);
                    cmd.CommandType = CommandType.StoredProcedure;
