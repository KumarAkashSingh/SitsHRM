...
public partial class Pages_DesignationTypeMaster : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Please Add Designation Type');ShowPopup3();", true);
        }
        ViewState["OpenFlag"] = 0;
        conn.Open();
        cmd = new SqlCommand("SP_Web_Masters", conn);
        cmd.Parameters.AddWithValue("@calltype", 29);
        cmd.Parameters.AddWithValue("@DesignationTypeName", txtDesignationType.Text);
        ...
        if (result == 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Designation Type Already exist');$('.modal-backdrop').remove();", true);
        }
        else
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Designation Type Added successfully','success');$('.modal-backdrop').remove();", true);
        }
    ...
    protected void GVDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "del")
        {
            ...
            ACID = GVDetails.Rows[RowIndex].Cells[1].Text;
            ViewState["ACID"] = ACID;
            Popup1(true);
        }
        else if (e.CommandName == "modify")
        {
            ...
            ViewState["ACID"] = ACID;
            ClearDetails();
            txtDesignationType.Text = GVDetails.Rows[RowIndex].Cells[2].Text;
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        cmd.Parameters.AddWithValue("@DesignationTypeName", txtDesignationType.Text);
        cmd.Parameters.AddWithValue("@DesignationTypeCode", ViewState["ACID"].ToString());
        cmd.Parameters.AddWithValue("@empcode", Session["e_code"].ToString());
        cmd.Parameters.AddWithValue("@OldValue", ViewState["OldValue"].ToString());
        SqlParameter parm = new SqlParameter("@ResultU", SqlDbType.Int);
        parm.Direction = ParameterDirection.ReturnValue;
        cmd.Parameters.Add(parm);
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.ExecuteNonQuery();
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Designation Type Updated successfully','success');$('.modal-backdrop').remove();", true);
        }
        ...
        foreach (GridViewRow row in GVDetails.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkRow.Checked)
                {
                    ...
                    cmd.Parameters.AddWithValue("@calltype", 31);
                    cmd.Parameters.AddWithValue("@empcode", Session["e_code"].ToString());
                    cmd.Parameters.AddWithValue("@DesignationTypeCode", row.Cells[1].Text);
                    cmd.CommandType = CommandType.StoredProcedure;
                    int result = cmd.ExecuteNonQuery();
                    cmd.Dispose();
                    conn.Close();
