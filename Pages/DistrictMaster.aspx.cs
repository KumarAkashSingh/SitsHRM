...
public partial class DistrictMaster : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Please Add District');ShowPopup();", true);
            return;
        }
        ...
        cmd.Parameters.AddWithValue("@District", txtDistrictName.Text);
        ...
        if (result == 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('District Already exist');$('.modal-backdrop').remove();", true);
            return;
        }
        else
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('District Added successfully','success');$('.modal-backdrop').remove();", true);
        }
    ...
    protected void GVDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "modify")
        {
            ...
            FillState();
            txtDistrictName.Text = GVDetails.Rows[RowIndex].Cells[2].Text.Replace("&amp;", "&");
            if (ddlState.Items.FindByText(GVDetails.Rows[RowIndex].Cells[3].Text) != null)
            {
                ddlState.Items.FindByText(GVDetails.Rows[RowIndex].Cells[3].Text).Selected = true;
            }
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        cmd.Parameters.AddWithValue("@DistrictID", ViewState["DC"].ToString());
        cmd.Parameters.AddWithValue("@StateID", ddlState.SelectedItem.Value);
        cmd.Parameters.AddWithValue("@District", txtDistrictName.Text);
        cmd.Parameters.AddWithValue("@EmpCode", Session["e_code"].ToString());
        ...
        if (result == 1)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('District Updated successfully','success');$('.modal-backdrop').remove();", true);
        ...
        foreach (GridViewRow row in GVDetails.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkRow.Checked)
                {
                    ...
                    cmd.Parameters.AddWithValue("@calltype", 4);
                    cmd.Parameters.AddWithValue("@EmpCode", Session["e_code"].ToString());
                    cmd.Parameters.AddWithValue("@DistrictID", row.Cells[1].Text);
                    SqlParameter parm = new SqlParameter("@Result", SqlDbType.Int);
                    parm.Direction = ParameterDirection.ReturnValue;
                    cmd.Parameters.Add(parm);
                    cmd.CommandType = CommandType.StoredProcedure;
