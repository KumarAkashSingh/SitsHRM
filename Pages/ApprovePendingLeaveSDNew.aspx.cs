...
public partial class Forms_ApprovePendingLeaveSDNew : System.Web.UI.Page
{
    ...
    protected DataTable FillGrid()
    {
        ...
        if (ddlFilterStatus.SelectedItem.Value != "0")
        {
            Criteria += " and ELM.LeaveStatus = '" + ddlFilterStatus.SelectedItem.Text + "'";
        }

        if (ddlEmployee.SelectedItem.Value != "0")
        {
            Criteria += " and ELM.E_Code = " + ddlEmployee.SelectedItem.Value;
        }

        if (ddlMonth.SelectedItem.Value != "0")
        {
            Criteria += " and Month(ELM.FromDate) = " + ddlMonth.SelectedItem.Value;
        }

        if (ddlYear.SelectedItem.Value != "0")
        {
            Criteria += " and Year(ELM.FromDate) = " + ddlYear.SelectedItem.Value;
        }
    ...
    protected void GVApprovedLeaveDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            //if (e.Row.Cells[5].Text != Session["empcode"].ToString() && e.Row.Cells[4].Text != "&nbsp;")
            //{
            //    string hex = "#82AF6F";
            ...
            if ((e.Row.Cells[20].Text == e.Row.Cells[21].Text) && e.Row.Cells[18].Text=="Approved")
            {
                string hex = "#82AF6F";
                e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(hex);
                e.Row.ForeColor = System.Drawing.Color.White;
            ...
            if ((e.Row.Cells[20].Text == e.Row.Cells[21].Text) && e.Row.Cells[18].Text == "Reject")
            {
                string hex = "#F89406";
                e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(hex);
                e.Row.ForeColor = System.Drawing.Color.White;
        ...
        foreach (GridViewRow row in GVApprovedLeaveDetails.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkRow.Checked)
                {
                    ...
                    //Check for Rejection of Leave
                    #region
                    if (ddlAction.SelectedItem.Value == "2")
                    {
                        conn.Open();
                        cmd = new SqlCommand("SP_ApproveLeave", conn);
                        cmd.Parameters.AddWithValue("@calltype", 4);
                        cmd.Parameters.AddWithValue("@status", ddlAction.SelectedItem.Text);
                        cmd.Parameters.AddWithValue("@ApprovedRemarks", txtRemarks.Text);
                    ...
                    //Check for Approval of Leave and Forward to Next Level according to below IF Clause
                    #region
                    else
                    {
                        if (row.Cells[4].Text != "" && row.Cells[4].Text != "&nbsp;")
                        {
                            //Status change from Apply To Approved
                            #region
                            if (row.Cells[18].Text == "Apply" || row.Cells[18].Text == "Approved")
                            {
                                if (row.Cells[5].Text == row.Cells[4].Text && row.Cells[6].Text == "True")
                                {
                                    conn.Open();
                                    cmd = new SqlCommand("SP_ApproveLeave", conn);
                                    cmd.Parameters.AddWithValue("@calltype", 4);
                                ...
                                else
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@CurrentLeaveAt", NextAuthority);
    ...
    protected void GVApprovedLeaveDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "fetch")
        {
            ...
            DateTime MDate = DateTime.ParseExact(GVApprovedLeaveDetails.Rows[RowIndex].Cells[16].Text, "dd MMM yyyy", CultureInfo.InvariantCulture);
            DateTime TDate = DateTime.ParseExact(GVApprovedLeaveDetails.Rows[RowIndex].Cells[17].Text, "dd MMM yyyy", CultureInfo.InvariantCulture);
            ViewState["FD"] = MDate;
            ViewState["TD"] = TDate;
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        cmd.Parameters.AddWithValue("@calltype", 49);
