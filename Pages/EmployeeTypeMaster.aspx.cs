...
public partial class Pages_EmployeeTypeMaster : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        if (txtEmpType.Text == "")
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Please Add Employee Type');ShowPopup3();", true);
        ...
        cmd.Parameters.AddWithValue("@Type", txtEmpType.Text);
        ...
        if (result == 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Employee Type Already exist');$('.modal-backdrop').remove();", true);
        }
        else
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Employee Type Added successfully','success');$('.modal-backdrop').remove();", true);
        }
    ...
    protected void GVDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "modify")
        {
            ...
            ViewState["ACID"] = ACID;
            ClearDetails();
            txtEmpType.Text = GVDetails.Rows[RowIndex].Cells[2].Text;
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        cmd.Parameters.AddWithValue("@Type", txtEmpType.Text);
        cmd.Parameters.AddWithValue("@EmpTypeCode", ViewState["ACID"].ToString());
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Employee Type Updated successfully','success');$('.modal-backdrop').remove();", true);
        }
        ...
        foreach (GridViewRow row in GVDetails.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkRow.Checked)
                {
                    ...
                    cmd.Parameters.AddWithValue("@calltype", 24);
                    cmd.Parameters.AddWithValue("@empcode", Session["e_code"].ToString());
                    cmd.Parameters.AddWithValue("@EmpTypeCode", row.Cells[1].Text);
                    cmd.CommandType = CommandType.StoredProcedure;
                    int result = cmd.ExecuteNonQuery();
                    cmd.Dispose();
                    conn.Close();
