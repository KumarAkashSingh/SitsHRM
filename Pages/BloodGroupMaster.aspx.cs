...
public partial class BloodGroupMaster : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Please Add Blood Group');ShowPopup();", true);
            return;
        }
        ...
        cmd.Parameters.AddWithValue("@BloodGroupName", txtBloodGroup.Text);
        ...
        if (result == 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Blood Group Already exist');$('.modal-backdrop').remove();", true);
            return;
        }
        else
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Blood Group Added successfully','success');$('.modal-backdrop').remove();", true);
        }
    ...
    protected void GVDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "modify")
        {
            ...
            ClearDetails();
            txtBloodGroup.Text = GVDetails.Rows[RowIndex].Cells[2].Text.Replace("&amp;", "&");
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        cmd.Parameters.AddWithValue("@BloodGroupCode", ViewState["DC"].ToString());
        cmd.Parameters.AddWithValue("@BloodGroupName", txtBloodGroup.Text);
        cmd.Parameters.AddWithValue("@EmpCode", Session["e_code"].ToString());
        ...
        if (result == 1)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Blood Group Updated successfully','success');$('.modal-backdrop').remove();", true);
        ...
        foreach (GridViewRow row in GVDetails.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkRow.Checked)
                {
                    ...
                    cmd.Parameters.AddWithValue("@calltype", 4);
                    cmd.Parameters.AddWithValue("@EmpCode", Session["e_code"].ToString());
                    cmd.Parameters.AddWithValue("@BloodGroupCode", row.Cells[1].Text);
                    SqlParameter parm = new SqlParameter("@Result", SqlDbType.Int);
                    parm.Direction = ParameterDirection.ReturnValue;
                    cmd.Parameters.Add(parm);
                    cmd.CommandType = CommandType.StoredProcedure;
