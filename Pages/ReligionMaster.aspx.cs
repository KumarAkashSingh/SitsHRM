...
public partial class Pages_ReligionMaster : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Please Add Religion');ShowPopup3();", true);
            return;
        }
        ...
        cmd.Parameters.AddWithValue("@ReligionName", txtReligionName.Text);
        ...
        if (result == 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Religion Already exist');ShowPopup3();", true);
            txtReligionName.Text = "";
            return;
        }
        else
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Religion Added successfully','success');$('.modal-backdrop').remove();", true);
        }
    ...
    protected void GVDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "modify")
        {
            ...
            ClearDetails();
            txtReligionName.Text = GVDetails.Rows[RowIndex].Cells[2].Text.Replace("&amp;","&");
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        if (txtReligionName.Text == "")
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Please Add Religion');ShowPopup();", true);
            return;
        }
        ViewState["OpenFlag"] = 0;
        conn.Open();
        ...
        cmd.Parameters.AddWithValue("@ReligionCode", ViewState["DC"].ToString());
        cmd.Parameters.AddWithValue("@ReligionName", txtReligionName.Text);
        cmd.Parameters.AddWithValue("@EmpCode", 1);
        ...
        if (result == 1)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$.notify('Religion Updated successfully','success');$('.modal-backdrop').remove();", true);
        ...
        foreach (GridViewRow row in GVDetails.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (chkRow.Checked)
                {
                    ...
                    cmd = new SqlCommand("SP_ReligionMaster", conn);
                    cmd.Parameters.AddWithValue("@calltype", 4);
                    cmd.Parameters.AddWithValue("@ReligionCode", row.Cells[1].Text);
                    SqlParameter parm = new SqlParameter("@Result", SqlDbType.Int);
                    parm.Direction = ParameterDirection.ReturnValue;
                    cmd.Parameters.Add(parm);
                    cmd.CommandType = CommandType.StoredProcedure;
