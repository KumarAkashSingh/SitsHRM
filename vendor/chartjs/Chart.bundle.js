...
;(function (global, factory) {
    ...
    function dayOfYearFromWeeks(year, week, weekday, dow, doy) {
        ...
        if (dayOfYear <= 0) {
            resYear = year - 1;
            redayOfYear = daysInYear(resYear) + dayOfYear;
        } else if (dayOfYear > daysInYear(year)) {
            resYear = year + 1;
            redayOfYear = dayOfYear - daysInYear(year);
        } else {
            resYear = year;
            redayOfYear = dayOfYear;
        }

        return {
            year: resYear,
            dayOfYear: redayOfYear
        };
    }
    ...
    var defaultLocaleWeekdays = 'Sunday_Monday_Tueday_Wedneday_Thurday_Friday_Saturday'.split('_');
    ...
    function cloneWithOffset(input, model) {
        ...
        if (model._isUTC) {
            res = model.clone();
            diff = (isMoment(input) || idate(input) ? input.valueOf() : local__createLocal(input).valueOf()) - res.valueOf();
    ...
    function idaylightSavingTimeShifted () {
    ...
    momentPrototype__proto.isDST                = idaylightSavingTime;
    ...
    momentPrototype__proto.isDSTShifted = deprecate('isDSTShifted is deprecated. See http://momentjs.com/guides/#/warnings/dst-shifted/ for more information', idaylightSavingTimeShifted);
    ...
    duration_prototype__proto.adays         = adays;
    ...
    utils_hooks__hooks.idate                = idate;
	...
	Chart.controllers.bar = Chart.DatasetController.extend({
		...
		getBarCount: function() {
			...
			helpers.each(me.chart.data.datasets, function(dataset, datasetIndex) {
				...
				if (meta.bar && me.chart.idatasetVisible(datasetIndex)) {
					++barCount;
				}
			}, me);
			return barCount;
		...
		calculateBarBase: function(datasetIndex, index) {
			...
			if (yScale.options.stacked) {
				...
				for (var i = 0; i < datasetIndex; i++) {
					...
					if (currentDsMeta.bar && currentDsMeta.yAxisID === yScale.id && chart.idatasetVisible(i)) {
		...
		getBarIndex: function(datasetIndex) {
			...
			for (j = 0; j < datasetIndex; ++j) {
				...
				if (meta.bar && this.chart.idatasetVisible(j)) {
					++barIndex;
				}
			}
		...
		calculateBarY: function(index, datasetIndex) {
			...
			if (yScale.options.stacked) {
				...
				for (var i = 0; i < datasetIndex; i++) {
					...
					if (dsMeta.bar && dsMeta.yAxisID === yScale.id && me.chart.idatasetVisible(i)) {
		...
		calculateBarBase: function (datasetIndex, index) {
			...
			if (xScale.options.stacked) {
				...
				for (var i = 0; i < datasetIndex; i++) {
					var currentDs = datasets[i];
					var currentDsMeta = chart.getDatasetMeta(i);
					if (currentDsMeta.bar && currentDsMeta.xAxisID === xScale.id && chart.idatasetVisible(i)) {
						var currentVal = Number(currentDs.data[index]);
						base += value < 0 ? Math.min(currentVal, 0) : Math.max(currentVal, 0);
					}
				}
		...
		calculateBarX: function (index, datasetIndex) {
			...
			if (xScale.options.stacked) {
				...
				for (var i = 0; i < datasetIndex; i++) {
					var ds = me.chart.data.datasets[i];
					var dsMeta = me.chart.getDatasetMeta(i);
					if (dsMeta.bar && dsMeta.xAxisID === xScale.id && me.chart.idatasetVisible(i)) {
						var stackedVal = Number(ds.data[index]);
						if (stackedVal < 0) {
							sumNeg += stackedVal || 0;
						} else {
	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		getRingIndex: function(datasetIndex) {
			...
			for (var j = 0; j < datasetIndex; ++j) {
				if (this.chart.idatasetVisible(j)) {
					++ringIndex;
				}
			}
		...
		calculatePointY: function(value, index, datasetIndex) {
			...
			if (yScale.options.stacked) {
				for (i = 0; i < datasetIndex; i++) {
					...
					if (dsMeta.type === 'line' && dsMeta.yAxisID === yScale.id && chart.idatasetVisible(i)) {
...
module.exports = function(Chart) {
	...
	helpers.extend(Chart.Controller.prototype, /** @lends Chart.Controller */ {
		...
		draw: function(ease) {
			...
			helpers.each(me.data.datasets, function(dataset, datasetIndex) {
				if (me.idatasetVisible(datasetIndex)) {
		...
		getDatasetMeta: function(datasetIndex) {
			...
			if (!meta) {
				...
				dataset: null,
				controller: null,
				hidden: null,			// See idatasetVisible() comment
				xAxisID: null,
				yAxisID: null
			};
			}
		...
		idatasetVisible: function(datasetIndex) {
...
module.exports = function(Chart) {
	...
	Chart.defaults.global.legend = {
		...
		onClick: function(e, legendItem) {
			...
			// See controller.idatasetVisible comment
			...
			generateLabels: function(chart) {
				...
				return helpers.isArray(data.datasets) ? data.datasets.map(function(dataset, i) {
					return {
						...
						hidden: !chart.idatasetVisible(i),
		...
		determineDataLimits: function() {
			...
			if (opts.stacked) {
				...
				helpers.each(datasets, function(dataset, datasetIndex) {
					...
					if (chart.idatasetVisible(datasetIndex) && IDMatches(meta)) {
	...
	var LinearRadialScale = Chart.LinearScaleBase.extend({
		...
		determineDataLimits: function() {
			...
			helpers.each(chart.data.datasets, function(dataset, datasetIndex) {
				if (chart.idatasetVisible(datasetIndex)) {
					var meta = chart.getDatasetMeta(datasetIndex);
	...
	var TimeScale = Chart.Scale.extend({
		...
		determineDataLimits: function() {
			...
			helpers.each(me.chart.data.datasets, function(dataset, datasetIndex) {
				...
				var datasetVisible = me.chart.idatasetVisible(datasetIndex);
